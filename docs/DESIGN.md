...
> **2026-08-15 の5件は、この設計書の数値表より `src/data/**` が正**(U48 / U50 / U52c / U63 / U67)。
> - **U67 CloudFront の払い出しの粒と、引き戻しの復活率**(ユーザー体感2件)。
>   ① 「CloudFront RUSH のクレジット払い出しが多すぎる」→ **1回の払い出しを 5〜50枚** の帯へ。
>      毎ゲームのキャッシュヒットは 25〜60枚 → **5〜50枚(90%は5〜20枚)**、
>      レア役の確定クレジットは 弱チェ6〜ゴースト300枚 → **弱チェ10〜確定役50枚**。
>      減ったぶんは **ヒット率 0.85 → 0.95 / 固定8G → 18G** で返し、
>      RUSH 1回の獲得は 中央値279 / 平均281 / p99 405枚 と他3種と横並びのまま。
>      副産物として **1ゲーム100枚超を作るチャネルはヒーローRUSHだけ**になった。
>   ② 「復活しすぎ」→ 引き戻し成功率 **0.82 → 0.55**(復活 0.25 → 0.16回/セッション)。
>      減らしたぶんは復帰先を **ゴーストボーナス(RUSH 45%)→ ゴーストボーナスSP(85%)**
>      へ上げて1回の価値で返した(= **復活したらラッキー**)。
>      ①②は同じラウンドで一体で着地させてある。**この2つは必ずセットで動かすこと**。
> - **U63 レア役の基準をさらに2倍**(ユーザー指示「通常モードを、いまの甘スロ相当に」)。
>   通常時テーブルのレア役を U48 のさらに2倍 = **従来比4倍** へ(弱チェ **1/12.5**・スイカ **1/25**・
>   チャンス目 **1/45**・強チェ **1/62.5**・サメ **1/300**・ゴースト **1/1500** = 合計 **1/6.17**)。
>   甘スロ(`?ama=1`)は倍率2のままなので **従来比8倍**(1/3.1)。
>   相殺は U48 と同じ手法(レア役1回あたりの変換率を絞る):
>   CZ当選 0.5倍 / ステージ昇格 0.7倍 / czPerGame 0.54倍 / フリーズ率(レア役)0.5倍 /
>   派生ゾーン 0.5倍 / RUSH 4種の上乗せは **非確定役だけ 0.5倍**(確定役は据え置き)/
>   Well-Architected CZ 9G → **7G** / Aurora RUSH 8G → **7G** /
>   通常時のベル払出 8 → **7枚**(コイン持ちの戻し)/ 天井 75G → **78G**(中央値の戻し)。
>   実測(U67 後 / 20,000セッション): 初当り **1/100.5** / 平均 **239.7枚** /
>   中央値 **139枚** / 上位1% **1,305枚** / 機械割 **179.9%** / フリーズ **9.1〜10.1%**。
>   甘スロは 初当り **1/61.4** / 平均 **386.5枚** / 機械割 **228.8%**。
...
>      1ゲーム最大 800 → 60枚 →(U67)**50枚**、ヒーローRUSH の +α 最大 1750 → 500枚 など)
>   ② **RUSH 1回の獲得は 中央値150〜300 / 平均250〜400 / p99 800枚以下**
>      (ゲーム数が伸びる AS と Aurora に通算上限 22G / 15G を新設)
>   上振れは「1回の大当たり」ではなく **引き戻しからの連チャン** で作る形へ移した
>   (U50 は引き戻し 82% × ゴーストボーナス。**U67 で 55% × ゴーストボーナスSP へ組み替え**)。
>   実測は `平均239.7枚 / 中央値139枚 / 上位1% 1,305枚 / 機械割179.9% / 初当り 1/100.5`。
...
>   TA 50% / SFN 55% / BlueGreen 62% / **Shield 66%** / FIS 72% / W-A 83%(参加型 / U63 で 7G)。
...
**M04. Well-Architected レビューCZ(7G / 公称突破率 83%)**
※ U27 で 10G固定にし、U48(レア役2倍)の相殺で 9G、U63(さらに2倍)の相殺で **7G** へ。突破率は獲得則から計算した公称値で、現行値は必ず `src/data/modes.js` の `CZ_TYPES.specs` を見ること。
...
> ・5G(初期値10Gから短縮)/ 成功率 **55%**(U50 で 82% まで上げたが、
>   U67 のユーザー体感「復活しすぎ」で U50 以前の 55% へ戻した)/
>   **レア役**成立で切替猶予 +1G(上限15G)
> ・**成功したら復帰先は「復旧のゴーストボーナスSP」**(U32 / U67)。以前の「元のRUSHへ直帰」は廃止。
>   引き戻し成功はRUSH確定ではなく、そのボーナス中にレア役を引けるか(**85%**)でRUSHへ戻る。
>   U67 で復帰先を S3_BIG(45%)から DYNAMO_BIG(85%)へ上げたのは、
>   **復活の回数を減らしたぶんを1回の価値で返す**ため(= 復活したらラッキー)
> ・失敗すると通常時(Free Tier)へ転落

**M18. ホットスタンバイ(Multi-AZ)(引き戻し / 5G / 成功率55%)**
ユーザー要望の引き戻しゾーン。RUSH終了時に必ず突入する。**AZ-a が落ちた状態から AZ-c へのフェイルオーバーを待つ**5ゲームで、成功すれば**復旧のゴーストボーナスSP**へ。液晶は2つのAZが並ぶ図で、待機系のインスタンスが起動していく様子がゲージで示される。「もう1台動いているから大丈夫」という Multi-AZ の安心感を、引き戻しの期待感に翻訳。RTO内(猶予ゲーム内)に復旧できるかが焦点で、滞在中にレア役を引けば猶予が1G伸びる。
...
| `CF_RUSH` | CloudFront RUSH | 毎ゲームの払い出し(固定18G / 1回5〜50枚) | 279 / 281 / 405枚 | 100枚(**確定役を含めても100枚**) | 25% |
| `AURORA_RUSH` | Aurora RUSH | 純増(ACU 30→70)の上げ幅 | 282 / 306 / 679枚 | 70枚 | 23% |
| `HERO_RUSH` | ヒーローRUSH | 5G固定の一発勝負 | 130 / 140 / 710枚(U76) | 80枚(確定役なら550枚) | 2%(プレミア契機は25%) |
...
**U76(2026-08-15 ユーザー指示)ヒーローRUSHを「瞬発プレミア」へ**:
毎ゲームの抽選を **80% × 70枚 → 50% × 50枚**(骨格 56 → 25枚/G)に半減した。
「何もしなくても5Gで280枚がほぼ確定」という骨格の重さそのものが指摘の対象だったため。
`hitCoin` が 70 → 50 になって1ゲーム100枚の壁に余白ができたので、
**非確定役の +α を U63 で半分にする前の値(弱チェ10 / スイカ・チャンス目18 / 強チェ30)へ戻し**、
1ゲームの最大は 50 + 30 = **80枚**(壁の内側)。期待獲得は 289 → **144枚**。
4種の中では一番軽いが、**中央値130枚に対して p99 710枚**(倍率5.5倍は4種で最大)で、
確定役を引いた1ゲームだけで 350〜550枚が出る = 上振れの尻尾でプレミアの格を保つ形にした。
なお期待値だけで見ると**プレミア振り分け(ヒーロー1/4)は通常振り分けより約13%軽い**が、
プレミア契機の価値は「ヒーローという見せ場を引けること」なので振り分けは据え置き
(理由と計算は `src/data/rushes.js` の `RUSH_TYPES` のコメント)。
...
- 実測(2026-08-15 / **U63 の着地後**。`--session=20000` × 2シード):
  甘スロ 初当り **1/62** / 機械割 **225%** / 平均 376枚、
  通常設定 初当り **1/99前後** / 機械割 **180〜181%** / 平均 241枚
  ※ U63 で通常設定のレア役が **1/6.17**(従来比4倍)になり、甘スロはそこからさらに2倍の **1/3.1**
- **甘スロの初当りは 1/62 が上限**(= これ以上は軽くならない)。
  CZ供給の主線が「上位ステージの滞在率 × `czPerGame`」で、滞在率は昇格率と転落率の釣り合いで
  決まるため **レア役を倍にしても滞在率は 1.27倍にしかならない**
  (実測の滞在 通常 44/42/15% → 甘スロ 29/46/25%)。
  1/45 まで持っていくと機械割が 260% 前後になり「甘スロ 250% 以内」とぶつかる。
  ここを動かすなら `czPerGame` に甘スロ専用の係数を足す設計変更が要る(ユーザー判断待ち)
