 ...
 *   alarm … Bedrock揃い(ALARM役)の起動イベント専用。CloudWatch アラームまわりの
 *           運用小ネタ。「揃った」こと自体の御祝儀で、当落は示唆しない
 *   mid   … 中。「何か起きている」ことは分かるが当選確定ではない
 *   gase  … ガセ。BONUS を打ち始めたように見えて別の単語に着地する肩透かし
 ...
 *      (src/data/scenarios/yokoku-heavy.js の yh_bedrock_typing_hit / _ready)
 ...
 * phase:0(打ち始め)で引いた文言を以降のフェーズが読み継ぐ必要がある。
 * @type {{tier:string, text:string}|null}
 */
let bedrockActive = null;

/** この再生で打つ文言を決める(phase:0 で引き直し、それ以外は継続) */
function bedrockLineOf(params) {
  ...
  if (phase <= 0 || bedrockActive == null || bedrockActive.tier !== tier) {
    bedrockActive = { tier, text: pickBedrockLine(tier) };
  }
...
export const LCD_ANIMS_EXTRA = {
   ...
   * までの短い導入(900ms)。この直後に bedrock_typing が生成を始める。
   * params: { color='#ffd166' }
   *
   * 配置: bedrock_typing のパネル(y44〜104)と同じ帯に重ねる。
  ...
  bedrock_boot: {
    ...
    draw(ctx, p, params, w, h) {
      ...
      const cx = w / 2;
      const cy = 74;
      const alpha = fadeInOut(p, 0.1, 0.62);
      if (alpha <= 0) return;
  ...
  /**
   * Bedrock タイピング予告。docs/BACKLOG.md「P: Bedrockタイピング予告」
   *
   * Bedrock のコンソール風パネルに、生成中の文言が1文字ずつ打ち出される。
   * リール停止に合わせて phase を 0 → 1 → 2 → 3 と上げると、
   * 打ち終わりが第3停止に重なる(= 何が生成されるかを最後まで引っ張れる)。
   *
   * params: { tier='weak', phase=0, revealSpan=0.18, ms }
   *   tier  … 'weak' | 'mid' | 'gase' | 'hot'。文言の期待度。
   *           **'hot'(BONUS を含む)は当選が確定したシナリオからしか渡さない**
   *   phase … 0〜3。表示する割合が (phase+1)/4 まで進む。0 で文言を引き直す
   *
   * 配置: y 44〜104。lcd.text(y 168〜236)と取り合わないので、
   *       打ち終わりの告知テロップは lcd.text 側へ任せられる。
   */
  bedrock_typing: {
    layer: 'ui', ms: 2600,
    draw(ctx, p, params, w, h) {
      ...
      // このフェーズで見せる範囲。phase:3 で 100% 打ち終わる
      const from = phase / 4;
      const to = (phase + 1) / 4;
      const k = easeOutCubic(clamp01(p / (params.revealSpan ?? 0.18)));
      const ratio = clamp01(from + (to - from) * k);
      const shown = line.text.slice(0, Math.max(1, Math.round(line.text.length * ratio)));
      ...
      // ヘッダ(サービス名と生成ステータス)
      strokedText(ctx, 'Amazon Bedrock', px + 10, py + 12, {
        size: 9, color: 'rgba(190,225,255,0.85)', edge: 'rgba(0,10,30,0.9)', align: 'left', heavy: false,
      });
      strokedText(ctx, done ? 'COMPLETED' : 'GENERATING…', px + pw - 10, py + 12, {
        size: 9, color: done ? color : 'rgba(190,225,255,0.6)', edge: 'rgba(0,10,30,0.9)',
        align: 'right', heavy: false,
      });
      ...
      // 本文(等幅寄りのフォントで「生成中」らしく見せる)
      const textY = py + 40;
      const maxW = pw - 28;
      let size = 14;
      ctx.font = `700 ${size}px ${FONT_MONO}`;
      if (typeof ctx.measureText === 'function') {
        while (size > 10 && ctx.measureText(shown).width > maxW) {
          size -= 1;
          ctx.font = `700 ${size}px ${FONT_MONO}`;
        }
      }
      const textW = typeof ctx.measureText === 'function' ? ctx.measureText(shown).width : 0;
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.lineWidth = 4.5;
      ctx.lineJoin = 'round';
      ...
      ctx.fillText(shown, tx, textY);

      // カーソル(打ち終わるまで点滅)
      if (!done && Math.floor(p * 12) % 2 === 0) {
        ctx.fillStyle = color;
        ctx.fillRect(tx + textW + 2, textY - size / 2, size * 0.5, size);
      }
