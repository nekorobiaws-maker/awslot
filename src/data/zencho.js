 ...
 * ところが AWSLOT には既に **赤文字予兆 = 信頼度 75〜85%** という別の意味の赤がある。
 * 同じ「赤」に2つの意味を持たせると、どちらの赤なのか読めなくなるので、
 * **意味ごとに見せ方のレイヤーを分ける** ことで両立させる。
 *
 *  ┌ 信頼度示唆 …… テキスト帯の tone
 *  │   tone:'hot' を付けたものだけが「赤文字予兆」。
 *  │   帯の下敷きごと赤くなり、文字が一段大きく脈打つ(lcdanims.js の TEXT_TONES.hot)。
 *  │   = 「帯ごと赤い」が信頼度のサイン。前兆の中で出すものは
 *  │     強度2以上 かつ 2G目以降でしか出さない。
 *  │
 *  └ 対応役示唆 …… 文字色(color)だけ
...
/* ══ 赤帯(tone:'hot')の信頼度設計 — U73(2026-08-16 再設計)══════════
 *
 * ユーザー指定の設計目標は **「赤帯が出たら 75〜85% で当たる」**。
 * 赤帯は1か所で出しているのではなく、意味の違う4系統の合計でこの数字を作る。
 * どれか1本の重み・chance・発火条件を触ったら **必ず全体を測り直すこと**。
 *   実測コマンド: node scripts/hot-trust-probe.mjs 1200 777 555 20260814 --human
 *   ※ --human は必須。即押しだとレバーON契機の予告が告知枠を握ったまま前兆の赤が来て、
 *     交通整理(director の DROP_TEXT)で赤のテキストが落ち、実機と違う数字になる。
 *
 * ── 4系統と実測値(3,600セッション / 360,000G / 標準設定)─────────
 *   系統            シナリオ                        占有率  信頼度   意味
 *   前兆の赤        zn_hot_*(16本)                 55.9%   93%    強度2以上・2G目以降。ガセ前兆でも出る
 *   伸びきった前兆  zn_final_push                   10.3%   52%    4Gガセが必ず通る = 裏切りの受け皿
 *   確定の先出し    yh_hot_bedrock_alert            13.2%  100%    チャンス目 / サメ揃い(CZ確定役)
 *   前兆の重なり    yh_hot_guardduty_alert          10.3%   82%    前兆中にレア役が重なった
 *   裏切り枠        yh_hot_false_alarm ほか1本      10.3%    1%    何も走っていないゲームの赤
 *   ─────────────────────────────────────────
 *   合計                                            100%  **79.3%**  ← 目標 75〜85% の内側
 *   赤の出現量: **1.08回/セッション**(1/92.5G)
 *
 * ── U72 で何が壊れ、U73 で何を直したか ───────────────────
 * U72 の「チャンス目 = CZ突入確定 / 他のレア役は cz 0」でCZ導線が一本化された結果、
 *   ・本前兆は「確定役の移行前兆(=100%)」が主流になり、前兆の赤が 93% まで上昇
 *   ・スイカに出していた赤は **CZへ繋がる道が消えて構造的にほぼ必ず裏切る** ようになった
 * この2つで赤の総合信頼度は **64.9%** まで落ちていた(目標を10ポイント以上下回る)。
 * U73 では **ゲーム側の数値には一切触れず**、赤の出し方だけを組み替えている:
 *   1. スイカ単独の赤を廃止(役に紐づく赤が毎回裏切る形を消す)
 *   2. 空いた枠を「CZ確定役の先出し(100%)」と「前兆中の重なり(82%)」へ配り直し
 *   3. 純粋な空振りの裏切り枠を 0.020 → 0.013 へ(赤全体の 15% → 10%)
 * → **64.9% → 79.3%**、赤の出現量は 1.14 → 1.08回/セッション(見せ場はほぼ維持)。
 *
 * ── 【重要】赤 = 確定にはしない ───────────────────────
 * 4系統のうち3系統(前兆の赤・伸びきった前兆・裏切り枠)は
 * **構造的に外れる道を持っている**。信頼度を上げたくなっても、
 * 「裏切り枠を消す」「zn_final_push の赤を外す」で稼ぐのは最後の手段にすること。
 * 赤が必ず当たる台になった瞬間、赤を見る楽しみそのものが消える。
 *
 * ── 甘スロ(?ama=1)について ───────────────────────
 * レア役が2倍なので **前兆に占める本前兆の割合そのものが上がり**、
 * 赤の信頼度は実測 **88%**(U73前は 74.5%)。標準設定を目標帯に置く限り
 * 甘スロが高く出るのは避けられないので、救済モードの性格として許容している。
 */
export const ZENCHO_TEXT_COLORS = {
  /** 信頼度示唆(tone:'hot' と必ずセットで使う色) */
  TRUST: {
    /** 赤文字予兆。実測の総合信頼度は 79%(目標 75〜85% の内側。上の設計コメント参照) */
    hot: '#ff3b30',
  },
 ...
 *
 * ■ ここを増減させたら赤の総合信頼度が動く(U73 / 2026-08-16)
 *   このリストが増えれば前兆の赤(信頼度 93%)の量が増え、赤全体の平均が上がる。
 *   目標は 75〜85% なので、足したら必ず
 *     node scripts/hot-trust-probe.mjs 1200 777 555 20260814 --human
 *   で測り直し、はみ出したら裏切り枠(yh_hot_false_alarm / yw_hot_false_evacuation)の
 *   chance で戻すこと。設計の全体像は上の ZENCHO_TEXT_COLORS のコメントにある。
 * @type {Set<string>}
 */
