...
export const AMA = {
  ...
  /**
   * 通常時テーブルのレア役に掛ける倍率。
   * 「1/N の N を割る」形で適用するので、2 なら 弱チェ 1/12.5 → 1/6.25
   * (U63 で通常設定そのものが従来比4倍になったので、甘スロは **従来比8倍**)。
   * ここを 3 以上にすると AS_RUSH の上乗せ期待値が 1.0 に近づいて
   * RUSH が理論上終わらなくなる(scripts/sim.mjs の検証26)。**2 が上限の目安**。
   *
   * 【甘スロの初当りが 1/45 まで軽くならない理由(U63 の実測と申し送り)】
   * 甘スロが動かすのは **レア役の出現率だけ** で、CZ供給の主線は
   *   「上位ステージの滞在率 × 毎ゲーム抽選(NORMAL_SUBSTATES.czPerGame)」。
   * 滞在率は昇格率と転落率の釣り合いで決まるので **レア役を倍にしても倍にはならない**
   * (実測 通常 44/42/15% → 甘スロ 29/46/25%。上位ステージ滞在は 56% → 71% = 1.27倍)。
   * その結果、初当りは 通常 1/99 に対して甘スロ **1/62**(1.6倍)で頭打ちになる。
   * ここを 1/45 まで持っていくには機械割が 260% 前後まで伸びて
   * 「甘スロは250%以内」という上限とぶつかるので、**1/62 で止めてある**。
   * それでも通常設定の 1.6倍・機械割 225% なので「壊さずに一番甘い」水準。
   */
  rareMultiplier: 2,
  ...
  /**
   * 実測の目安(U63 着地後 / 20,000セッション × 2シード)。
   * 表示担当がバッジの補足に使えるよう、通常設定との差を数字で持っておく。
   *   通常設定 … 初当り 1/99  / 平均 241枚 / 中央値 156枚 / 機械割 181%
   *   甘スロ   … 初当り 1/62  / 平均 376枚 / 中央値 255枚 / 機械割 225%
   */
  measured: { hitDenom: 62, meanScore: 376, medianScore: 255, payoutRate: 2.25 },
};
