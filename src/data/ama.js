...
export const AMA = {
   ...
   *
   * 【U72 で頭打ちが解消された経緯】
   * U63 時点では CZ供給の主線が「上位ステージ滞在率 × 毎ゲーム抽選」で、
   * レア役を倍にしても滞在率は 1.27倍にしかならず、甘スロは 1/62 で頭打ちだった。
   * U72 で主線が「チャンス目(CHANCE)成立 = CZ確定」になったため、
   * レア役2倍がそのまま CZ 2倍に直結するようになり、実測 1/53 まで軽くなった
   * (機械割 235% で上限 250% の内側。これ以上軽くしたければ rareMultiplier ではなく
   *  チャンス目の出現率か CZ 突破率の甘スロ専用係数を検討すること)。
   */
  rareMultiplier: 2,
  ...
  /**
   * 実測の目安(U72 着地後 / 20,000セッション × 3シード)。
   * 表示担当がバッジの補足に使えるよう、通常設定との差を数字で持っておく。
   *   通常設定 … 初当り 1/96  / 平均 258枚 / 中央値 145枚 / 機械割 186%
   *   甘スロ   … 初当り 1/53  / 平均 405枚 / 中央値 250枚 / 機械割 235%
   */
  measured: { hitDenom: 53, meanScore: 405, medianScore: 250, payoutRate: 2.35 },
};
