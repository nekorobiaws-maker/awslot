 ...
 *     → **U72(2026-08-15)でこの申し送りを採用**(下記)。
 *
 * ── 【U72(2026-08-15)/ ベル 15 → 18枚】────────────────────────
 * 同じラウンドで
 *   ・オートスケーリングRUSH の純増を 35 → 15枚/G(ユーザー指示)
 *   ・CZ突破率ラダーを一律 0.80倍(チャンス目=CZ確定でCZ突入が1.5倍になった相殺)
 * を入れた結果、**セッションの上振れ(上位1%)だけが目標(1,250枚)を割った**。
 * AS が「長い×軽い」になったぶん 100回転という持ち時間の中で連鎖できる回数が減り、
 * 上位のセッションほど時間切れになるのが原因(AS 1回の価値そのものは目標帯のまま)。
 *
 * 上振れは **1セッションに何回ボーナスを重ねられたか** でできているので、
 * ボーナス1回の厚みで返すのが一番副作用が少ない(初当りの確率には一切触れない):
 *   期待払出 11.45 → **13.60枚/G** = 純増 8.45 → **10.60枚/G**(+25%)
 *     18×(1/1.4) + 3×(1/20) + 2×(1/10) + 2×(1/60) + 5×(1/18)
 *     + 2×(1/28) + 3×(1/400) + 0×(1/6000) ≒ 13.60枚/G
 *   シャークボーナス(6G)51 → **64枚** / ゴーストボーナス(8G)68 → **85枚** /
 *   ゴーストボーナスSP(平均12G)101 → **127枚**
 * = 「CZは抜けにくくなったが、抜けたあとのボーナスは厚い」という形で釣り合わせている。
 *   実測(8,000セッション×2シード): 上位1% 1,211 → **1,265〜1,273枚**(目標1,250超)/
 *   平均 230 → **253枚** / 機械割 177 → **184%**(いずれも目標帯の内側)
 * 17枚(純増 9.88)では上位1% が 1,248枚と目標にわずかに届かなかったため 18枚を採用。
 * ボーナス中の **出現率(denom)は1つも触っていない** ので、
 * RUSH当選率(12 / 45 / 85%)と data/rushes.js の bonusMult は引き直し不要。
 *
...
export const BONUS_FLAGS = {
  ...
  bet: 3,
  flags: [
    { id: 'BELL',          name: 'ベル(EC2)',           denom: 1.4,  payout: 18, rare: false },
  ...
  /** U72(上振れの補填)の直前のベル払出。純増 8.45枚/G だった頃の値 */
  previousBellPayout15: 15,
  /** U72 の検討で通過した中間値(純増 9.88枚/G。上位1%が1,248枚で目標に届かず) */
  previousBellPayout17: 17,
