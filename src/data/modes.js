 ...
 *
 * 【なぜ復帰先(S3_BIG / 45%)は上げなかったか】※ U67 で方針転換した。下を読むこと
 ...
 *
 * ── U67(2026-08-15 ユーザー体感「復活しすぎ」)/ 0.82 → **0.55** ──────────
 *
 * U50 の「上振れを連チャンで作る」は数字の上では成立していたが、
 * **82% は5回に4回戻ってくる**ので、引き戻し層が「当落を見る画面」ではなく
 * 「戻るのを待つだけの画面」になっていた(実測 復活 0.26回/セッション)。
 * 引き戻しは1回のRUSHにつき必ず1度通る唯一の防衛線なので、
 * ここの当落が形骸化すると台全体で一番よく見る抽選が死ぬ。
 *
 * U50 以前の **0.55** へ戻す(= 復活 0.16回/セッション。約1/3 減)。
 * ただし単に下げると U50 が作った上振れが崩れる(実測: 上位1% 1,279 → 1,186枚 /
 * 平均 239 → 222枚)ので、**減らしたぶんを「1回の価値」で返す**:
 *
 *   復帰先を S3_BIG(ゴーストボーナス / RUSH 45%)
 *         → **DYNAMO_BIG(ゴーストボーナスSP / RUSH 85%)** へ
 *           (正は data/rushes.js の RECOVERY_BONUS)
 *
 *   復活1回の価値 ≒ 78 + 0.45×290 ≒ 209枚 → **110 + 0.85×290 ≒ 357枚**
 *
 * = 「復活はしにくいが、復活したら上位ボーナス」。ユーザーの言う
 * **「復活したらラッキー」** の体感はこの形でしか作れない
 * (頻度を下げたうえで価値も据え置くと、ただ痩せるだけになる)。
 *
 * 【U50 の「DYNAMO_BIG にすると機械割 196%」はなぜ再現しないか】
 * あの実測は **成功率 0.82 のまま** 復帰先を上げた場合の話。
 * 連チャン率は 0.82×0.45 = 37% → 0.55×0.85 = **47%** と上がるが、
 * 復活の発生自体が 0.82 → 0.55 に減るので、
 * セッション全体のRUSH突入回数は 0.56 → 0.57回 と **ほぼ動かない**
 * (実測 機械割 179.7% → 180.7% / 平均 239 → 242枚 / 上位1% 1,279 → 1,278枚)。
 * **成功率と復帰先はセットで動かすこと。片方だけ動かすと必ずどちらかへ振り切れる。**
 */
  ...
  specs: [
    {
       ...
       * **平均延長 0.87G / 平均滞在 5.87G**(引き戻し成功は successRate どおり)。
       * 5G中に1回は延びる(1 − 0.838^5 = 59%)ので「延長が日常」寄りに戻っているが、
       *   ・延長は **当落を一切動かさない**(successRate は 5G でも 9G でも同じ)
      ...
      previousGames: 10,
      /**
       * 引き戻し成功率。U67(ユーザー体感「復活しすぎ」)で 0.82 → **0.55**。
       * 下げるときは **必ず data/rushes.js の RECOVERY_BONUS.bonusId とセットで**
       * 動かすこと(上の RECOVERY_SPECS ヘッダの U67 の項を参照)。
       */
      successRate: 0.55,
      /** U67(復活しすぎの是正)の直前値。上振れを連チャンで作っていた頃 */
      previousSuccessRate082: 0.82,
