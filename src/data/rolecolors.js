 ...
 *
 * ══ 文言の3条件(U25 → U66 → U67 → U78 の積み上げ)═══════════════════
 *
 * 液晶に出す文字は、**結論行かどうかに関わらず** 次の3つを全部満たすこと。
 * 迷ったら「音声だけで読み上げられても意味が通るか」で判定する。
 *
 *   ① **初見で意味が通るか(絵に依存しない)** ← U78 で追加
 *   ② **AWS に関係があるか**
 *   ③ **事実に反しないか**
 *
 * ── ① 絵に依存する比喩を書かない(2026-08-15 U78 ユーザー指摘)──────────
 *
 * 【指摘】「『金の粒は流れてこなかった…』が意味不明」
 * 【正体】「金の粒」は Kinesis アニメで流れている **粒の色** を指した言葉だった。
 *         書いた側は画を見ながら書くので通じるが、
 *         プレイヤーは文字を読む瞬間に画のどこを見ているか分からないし、
 *         そもそも「粒が金 = 期待度が高い」という対応表をどこでも教えていない。
 * 【ルール】アニメの視覚要素(色・形・動き)を **そのまま名詞にして書かない**。
 *         書くのは「その絵が表している出来事」のほう。
 *           ×  金の粒は流れてこなかった   /  青いまま流れていった
 *           ×  虹の風が運んできた         /  手がかりの糸が切れた
 *           ○  データが流れただけで終わった(Kinesis — 拾うべきレコードは無かった)
 *           ○  CloudFront のエッジからアクセスキーが届いた
 *         色そのものが情報を持つのは **役色(上の ROLE_COLORS)だけ**で、
 *         そちらは「文字の色」であって「文の中身」ではない。
 *
 * ── ② AWS に関係があるか ──────────────────────────────────────
 *   1行(メイン + サブ)のどちらかに **サービス名か、その実態を指す言葉** を必ず入れる。
 *   テキスト帯は1件ずつ順送りなので、導入行にサービス名があっても
 *   結論行が出るころには消えている。**行ごとに** 成立させること。
 *     ×  RAINBOW / 上乗せ濃厚          (パチスロの機能語しかない)
 *     ○  全シャードが最大まで開いた / Amazon Kinesis — 流量が跳ね上がった。上乗せ濃厚
 *   例外は **台の進行そのものを告げる行** だけ(「100 GAMES FINISH」
 *   「ゴースト7を揃えろ!」など)。ここに AWS を足すと操作指示が読みにくくなる。
 *
 * ── ③ 事実に反しないか ──────────────────────────────────────
 *   AWS の仕様として正しいこと。断定できない予告は断定しない
 *   (低信頼度の推定なら「当たっているとは限らない」と書き添える)。
 *   専門用語(DLQ / BGP / 子の実行 など)は **その行の中に一言の日本語補足**を置く。
 *     ×  DLQ を空にした
 *     ○  デッドレターキュー(DLQ)を空にした
 *
