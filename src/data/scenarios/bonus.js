...
export default [
...

  {
     ...
     *
     * ■ U74(2026-08-15): 「キャパシティ確保がすぐ消えて読めない」
     *   ユーザー指摘。ジャッジの大文字(capacity_judge の結論)は
     *   **アニメの尺の最後 0.78秒ぶんしか出ていない**(p 0.70〜1.00 / ms 2600)。
     *   セット継続という一番うれしい結果が、読む前に消えていた。
     *
     *   直し方は U57 の結論ライフサイクルに揃える:
     *     ・ヘルスチェック → キャパシティチェック → 結論 の **リズムは一切触らない**
     *       (アニメの ms もキューの時刻も従来どおり)
     *     ・アニメが消えぎわに入るところで **結果の1行だけ** をテキスト帯へ移し、
     *       sticky で **次のレバーONまで残す**(lcdanims.js の releaseSticky)
     *   これで結論は 1.82秒(大文字)→ 以降ずっと帯、と途切れず読み続けられる。
     */
    ...
    cues: [
       ...
       * ただしそれはアニメの尺(2600ms)と一緒に消えてしまうので、
       * **アニメが消えぎわに入る 2400ms**(結論の大文字は 1820ms から出ている)で
       * 同じ文言をテキスト帯へ引き継ぎ、sticky で次のレバーONまで残す。
       *
       * ■ 2箇所で同時に言っていないか(U8)
       *   アニメ側の alpha は fadeInOut(p, 0.05, 0.94) = **2444ms から薄れ始める**。
       *   帯も出現に TEXT_APPEAR_MS(170ms)かけて立ち上がるので、
       *   重なるのは実質クロスフェードの一瞬だけで、二重表示にはならない。
       *   ここより前に置くと大文字と帯が **同時に同じことを言う** ので下げないこと。
       *
       * ■ サブ行を付けない理由
       *   「SET n へ」を書きたくなるが、ボーナス盤面が常設で `SET n` を出しており
       *   (render/lcd.js の _drawBonus。帯が出ている間は y130 へ逃がす)、
       *   同じ数字が上下に並ぶ。持続情報は盤面・瞬間の告知は帯、の役割分担どおり
       *   ここは結論の1行だけを持つ。
       *
       * ■ 盤面の常設表示(獲得枚数 / 残りG / SET)との重なり
       *   帯が出ると _drawBonus が textActive レイアウトへ切り替わり、
       *   数値は帯の上(y104 / y130)へ逃げる。SET継続の帯が居座っても隠れない。
       */
      { at: 2400, layer: 'lcd',  action: 'text',
        params: {
          text: 'キャパシティ確保 — 継続!!', color: '#7bf7d0', ms: 2600, sticky: true,
        } },
    ],
  },
