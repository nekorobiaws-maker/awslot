 ...
 * 冠のタイトル・レバー球・MAX BET・赤ボタン・下部パネルは全部 0.76 の等倍のまま。
 *
 ...
 *
 * 2026-08-15 ルナ版アートへの差し替え(3回目)で **この表は 1px も変えていない**。
 * 新旧2枚を帯ごとにずらして重ね(±6pxの総当たりで平均絶対差が最小になる
 * ずれ量を探す)検算したところ、冠 / 側面LED帯 / リール帯 / SUMMIT / INVENT METER /
 * 操作デッキ / 下部パネル枠 / 台座の **すべてが dy=0, dx=0 で最小**だった。
 * 個別の実測でも 停止ボタン中心 (437.5, 1025.5)(539.5, 1025.5)(640.5, 1025.5)、
 * リール窓のオレンジ枠 x318..437 / x482..603 / x647..764、デッキの黒い凹み
 * y881..964、下部パネルのLED帯 x146..168 / x912..931 と、前アートと同じ値になる。
 * 変わったのは「上部液晶の中身が黒無地になった」ことと「下部パネルの絵柄が
 * ルナ+AWS六角アイコンになった」ことだけなので、バンド表・窓・style.css の
 * DOMヒットエリアは全部据え置き。クロマキーの keep だけ増やしてある。
 */
 ...
 *          逆に開口より内側に取ると端数の丸めで焼き込みアートが1px覗く)
 ...
 *
 * 2026-08-15 ルナ版アート(液晶が黒無地)でも **この表は据え置き**。
 * 焼き込みアートが無くなったぶん黒い面が広がって x≒186..898 / y≒171..588 まで
 * 続いているが、その中心は (542, 379.5) で窓の中心 (544, 378) とほぼ一致する。
 * 窓を黒面いっぱいまで広げるには 712:417 ≒ 1.71 という横長比が要り、
 * 液晶Canvasの論理比 440:300 = 1.4667 と合わない(広げるなら帯の再配分=下流全部が
 * 動く)ので、**窓は今までどおり黒面の内側に置く**。差の 左右≒20px / 上下≒3px は
 * 黒面の上に黒が乗るだけなので、実機ではベゼルの一部に見える。
 * 下部パネルのルナ(元絵 y1072..1318)は最下段の Canvas である HUD 窓の下端
 * (元絵 y962.2)よりずっと下なので、どの Canvas にも隠れない。
 */
const CABINET2_WINDOWS = {
  /*
   * ── 2026-08-16 検証指摘 V80-2「液晶ベゼルが左右非対称」──────────────────
   * 元絵の黒面(液晶の開口)は論理座標で x91.33〜633.67
   * (`node scripts/measure-bezel.mjs` の実測。液晶の高さ方向 38行の中央値)。
   * 窓 x1=874 のときの余白は
   *   左 20.14px / 右 20.60px  = 0.47px ぶん右へ寄っていた。
   * x1 を 874.6 にすると左右がぴったり揃う(窓は 0.46 論理px だけ広がるだけで、
   * 高さ・リール窓・HUD窓・バンド表はどれも動かない = 下流に影響しない)。
   *
   * ※ これで **窓は黒面のど真ん中**になる。プレイ中に残る左右差は
   *   筐体アートのLED帯が明滅して片側だけ明るく見える瞬間のもので、
   *   窓の位置ではなく assets/ui/cabinet2.png 側の絵の話。
   */
  lcd:   { x0: 214, y0: 174,   x1: 874.6, y1: 582 },
 ...
 * 絵として描かれた緑は明示的に守る必要がある。守っているのは3か所:
 *   1. 液晶窓。前アート(サメ)では周りの緑のAWSアイコンを守るために要った枠。
 *      2026-08-15 のルナ版は液晶が黒無地なので中に緑は無いが、
 *      将来また焼き込みアートが戻ったときのために従来どおり残す(実害なし)
 *   2. INVENT METER の緑セグメント(実測 x839..879 / y838..880)。
 *      守らないとメーターの下3目盛りが四角く抜け落ちる
 *   3. **下部パネル全面**(2026-08-15 追加)。ルナ版で増えた
 *      **S3バケツの緑六角(実測 x241..313 / y1154..1236)** がここにある。
 *      六角だけを囲うより枠ごと守るほうが安全で、パネルの光条にある
 *      緑がかった細い筋(x308/x380/x887 あたりの1px線)まで一緒に守れる。
 *      パネルの中に背景は1画素も無いので、広く取っても抜け残りは増えない
 * 逆に keep 矩形の中に「抜きたい背景の緑」が入らないことも確認済み
 * (3つとも筐体の内側で、背景の緑とは接していない。連結成分で数えても
 *  背景成分の画素は3矩形とも 0 個)。
 */
const CABINET2_CHROMA = {
  ...
  keep: [
    ...
    // INVENT METER の緑セグメント(メーターの下3目盛り)
    { x0: 832, y0: 836, x1: 886, y1: 886 },
    // 下部パネル(ルナ + AWS六角アイコン群。緑はS3バケツの六角)
    { x0: 170, y0: 1066, x1: 912, y1: 1322 },
  ],
};
...
const CABINET_ARTS = [
  {
    ...
    /** 液晶の面を黒で潰す(ルナ版は元から黒無地なので保険)。窓と同じ矩形 */
...
export class CabinetView {
   ...
   * ルナ版(2026-08-15)の液晶は元から黒無地なので実質は無害な二度塗りだが、
   * 焼き込みアート入りの絵に戻したときも「窓の中は必ず黒」を保証してくれる
   * (グリーンバックを抜いた画像では keep した緑がそのまま残るため)。
   */
  _fillScreen(ctx) {
